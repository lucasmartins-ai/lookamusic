...
- **P16 Sample instruments** — SampleVoice + SampleCache + packs (Salamander piano CC-BY, FreePats nylon CC0, Salamander drums opcional), synth as fallback. Done when: quintet sounds real with packs, byte-identical fallback without; licenses credited; pack weights within budget.
